...
module Fiasco
  class UndeclaredTemplate < Exception; end
  class Render
    if MP::Conf::DEVELOPMENT
      # Override Entry to allow read-on-the-fly template when rendering
      class Entry
        attr_accessor :body, :filename

        def initialize(body, filename)
          @body = body
          @filename = filename
        end

        def body; File.read(@filename); end
      end
    else
...
EOS
    ...
    def _render(name, locals = {})
      ...
      unless @compiled.include?(name) && !MP::Conf::DEVELOPMENT
    ...
    def render(name, locals = {})
      if !@templates.include?(name.to_sym)
        raise Fiasco::UndeclaredTemplate, "Template \"#{name}\" was not declared."
      end
      _render(name, locals)
    ensure
    ...
    def load_macros(options)
      @render_output, tmp = [], @render_output
      b = binding
